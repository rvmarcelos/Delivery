...
fit_knn <- knn.reg(train = df_knn_train, 
               test = df_knn_test, 
               y = df_knn_train$Output, 
               k = k_otimo)  




df_pred_knn <- data.frame(Estimado = fit_knn$pred,
                          Observado = df_knn_test$Output)

df_pred_knn %>% roc_auc(truth = as.factor(Observado), estimate = Estimado)
df_pred_knn %>% rmse(truth = Observado, estimate = Estimado)
